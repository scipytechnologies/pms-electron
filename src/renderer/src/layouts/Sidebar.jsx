...
import { dashboardMenu, applicationsMenu, pagesMenu, Menu } from '../data/Menu'
    ...
    return (
      <div className="sidebar">
        <div className="sidebar-header">
          {/* <Link to="/" className="sidebar-logo">PMS1</Link> */}
          <Image style={{ height: '60px' }} src={logo} fluid={true} alt="..." />
        </div>
        ...
        <p className='d-flex justify-content-end align-items-end' style={{height:'100%', fontSize:'9px',color:'white'}}>Beta v0.2.0</p> 
        </div>
      </div>
    )
  }
...
class SidebarMenu extends Component {
  ...
  toggleSubMenu = (e) => {
    ...
    while (node) {
      if (node !== parent && node.nodeType === Node.ELEMENT_NODE) node.classList.remove()
